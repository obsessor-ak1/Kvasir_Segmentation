...
from seg_modules.architectures.unet.layers import (
...
class UNet(nn.Module):
    ...
    def __init__(self, in_channels, num_classes=5, depth=5):
        ...
        self.final_downsample = UNetDownsampleConvBlock(
            channels[depth - 2], channels[depth - 1]
        )
    ...
    def __init__(self, in_channels, num_classes=1, inf_avg=True, depth=5):
        ...
        self.skip_nodes = nn.ModuleList(
            [
                nn.ModuleList(
                    [
                        UnetPlusPlusSkipNode(level=i, skip_id=j)
        ...
        self.upsample_convs = nn.ModuleList(
            [
                UNetUpsampleBlock(
                    channels[depth - j - 1] * (j + 1), channels[depth - j - 1]
                )
